...
export default function PublicBookingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === "manual") {
      ...
      // Calcular valor e enviar cobrança PIX via WhatsApp (uazapi /send/request-payment)
      ...
      if (clientPhone && (profile as any).manual_pix_key) {
        try {
          ...
          if (!pixRes.ok || pixJson?.success === false) {
            ...
            console.log("[PIX WhatsApp] Enviado com sucesso!");
          }
        } catch (e) {
          console.error("[PIX WhatsApp] Exceção:", e);
        }
      } else {
        console.warn("[PIX WhatsApp] Pulado — telefone ou chave PIX da arena ausentes.", {
          clientPhone,
          pixKey: (profile as any).manual_pix_key,
        });
      }
